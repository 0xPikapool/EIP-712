    ...
    while let Some(item) = types.pop() {
        if let Some(fields) = message_types.get(item) {
            ...
            for field in fields {
                ...
                if deps.contains(&*field.r#type) || !message_types.contains_key(&*field.r#type) {
                    continue;
                }
                types.insert(&*field.r#type);
            }
        }
    }
        ...
        .filter_map(|dep| {
            message_types.get(dep).map(|field_types| {
                let types = field_types
                    .iter()
                    .map(|value| format!("{} {}", value.r#type, value.name))
                    .join(",");
    ...
    let encoded = match message_type {
        ...
        Type::Custom(ref ident) if message_types.get(&*ident).is_some() => {
                ...
                .expect("Already checked in match guard; qed")
            {
                ...
                let r#type = parser.parse_type(&*field.r#type)?;
                let mut encoded =
                    encode_data(parser, &r#type, &message_types, &value, Some(&*field.name))?;
